...
**应用名** 默认为 DeepSeek Harness
- 会显示在：窗口标题栏、开始菜单/桌面快捷方式、安装包文件名、"关于"对话框
- 支持中英文、空格
- 版本号不支持自定义，固定为 `0.1.0`

**② 图标路径** — **直接回车会弹出文件选择框**，选一张图片即可（若着急使用可选择build文件夹中的ico文件）；也可以直接输入路径。**要求是严格的**，不符合会报错中止：
...
- 只接受这两种格式作为图标，其他格式目前不支持
- PNG 不是正方形、边长超出 256~1024、ICO 里没有大尺寸图像，向导都会报错并中止；
- 在文件选择框里取消 = 保留占位图标；若不满意可重跑 `npm run diy` 更换。
...
双击启动 → 先探测 `127.0.0.1:3080`（或 `DSH_PORT`/`--port` 指定端口）：**已有 dsh 实例在运行就直接复用**，绝不启动第二个实例
（两个实例并发写同一会话日志会造成 seq 撞号、日志损坏）→ 没有实例时才由 `server.js` 定位 npx 缓存中的
`@deepseek-ai/dsh` 入口用 `node` 拉起（`dsh web --port 0`）→ 解析它打印的 `dsh web: http://...` URL →
`main.js` 用 Electron 窗口加载该 URL，标题栏固定显示 DIY 应用名 → 关闭窗口时若为自启实例，
稍等写入收尾后用 `taskkill /T` 清理整个服务进程树（含 npx 链路的双保险清扫）；复用的实例则保持不变。
...
- **网页版（127.0.0.1:3080）和桌面版同时开着会损坏历史？** 旧版本会在已有实例之外再启动一个 dsh，两个实例并发写同一会话日志导致损坏（seq 撞号）。**新版本已修复**：启动时先探测，已有实例直接复用，不再启动第二个。
